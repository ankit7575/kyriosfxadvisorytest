...
const ForgotPasswordPage = () => {
  ...
  const sendResetEmail = (email, code) => {
    if (process.env.NODE_ENV === "development") {
      console.log(`Sending password reset code ${code} to email: ${email}`); // Log only in development mode
    }
  };
  ...
  const handlePasswordReset = (e) => {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log("Password has been reset for:", email); // Log only in development mode
    }
    navigate("/login");
  };
